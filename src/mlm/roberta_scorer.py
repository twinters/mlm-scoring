...
class MLMScorerRoberta:
    ...
    def __init__(
        ...
        self._wwm = wwm
        self._add_special = _add_special
    ...
    def _ids_to_masked(
        ...
        if self._wwm:
            for idx, token_id in enumerate(token_ids):
                if self._tokenizer.is_first_subword(
                    self._tokenizer.convert_ids_to_tokens(token_id)
                ):
        ...
        mask_token_id = self._tokenizer.convert_tokens_to_ids(
    ...
    def print_record(self, record):
        readable_sent = [
            self._tokenizer.convert_ids_to_tokens(tid) for tid in record[1]
        ]
        logging.info(
            """
sent_idx = {},
...
masked_id = {}
    ...
    def corpus_to_dataset(self, corpus: List[str]) -> SimpleDataset:
        ...
        for sent_idx, sent in enumerate(corpus):
            ...
            if self._add_special:
                ...
                tokens_original = self._tokenizer(sent)["input_ids"]
            ids_original = np.array(tokens_original)
    ...
    def score(
        ...
        for batch_id, batch in enumerate(dataloader):
            ...
            #             batch_compo, 1, batch_axis=0, even_split=False
            #         )
            ...
            masked_positions = torch.as_tensor(masked_positions)
